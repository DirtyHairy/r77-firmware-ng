			...
			SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	int dst_locked;
printf("%s %d\n", __func__, __LINE__);
printf("src[%p], w=%d, h=%d, dst[%p], w=%d, h=%d\n", src, srcrect->w, srcrect->h, dst, dstrect->w, dstrect->h);
...
printf("%s %d blit[%p] s_skip=%d, d_skip=%d, s_format=%d, d_format=%d\n", __func__, __LINE__, RunBlit, info.s_skip, info.d_skip, src->format, dst->format);
		RunBlit(&info);
	}
...
#endif
	printf("%s w=%d, h=%d, srcskip=%d, dstskip=%d\n", __func__, w, h, srcskip, dstskip);
	while ( h-- ) {
...
printf("%s\n", __func__);
	...
	srcskip = w+info->s_skip;
	dstskip = w+info->d_skip;
printf("%s w=%d, h=%d, srcskip=%d, dstskip=%d, src=%d, dst=%d\n", __func__, w, h, srcskip, dstskip, src, dst);
	if ( dst < src ) {
		while ( h-- ) {
